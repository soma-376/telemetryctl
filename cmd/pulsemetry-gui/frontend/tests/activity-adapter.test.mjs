...
const formatSource = readFileSync(
  new URL("../src/lib/utils/format.ts", import.meta.url),
  "utf8",
);
const formatJS = ts.transpileModule(formatSource, {
  compilerOptions: { module: ts.ModuleKind.ESNext },
}).outputText;
const formatURL = `data:text/javascript;base64,${Buffer.from(formatJS).toString("base64")}`;
const source = readFileSync(
  new URL("../src/pages/activity/adapter.ts", import.meta.url),
  "utf8",
);
...
const { sessionRow, sessionDetail } = await import(
  `data:text/javascript;base64,${Buffer.from(compiled.replace("$lib/utils/format", formatURL)).toString("base64")}`
);
...
test("토큰 합계를 축약하고 제목을 폴백한다", () => {
  ...
  assert.equal(
    sessionRow({ ...row, cost_usd: 0.135, reported_cost_calls: 1 }).cost,
    "0.14$",
  );
  ...
  for (const hour of [0, 9, 21]) {
    ...
    assert.equal(
      sessionRow({ ...row, started_at }).time,
