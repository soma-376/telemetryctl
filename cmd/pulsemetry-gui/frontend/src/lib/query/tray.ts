...
import { noteFailure, noteSuccess } from "$lib/domain/reconnect";
...
// useTrayQuery 는 데몬이 들고 있는 상태를 읽는다. 벤더를 두드리지 않는다.
//
// visible 이 false 면 폴링을 멈춘다. 트레이 창은 닫혀도 파괴되지 않고 숨겨질 뿐이라,
// 이것을 끄지 않으면 보이지도 않는 창이 1분마다 데몬을 깨운다.
export function useTrayQuery(q: TrayQuery, visible: boolean) {
  return useQuery({
    queryKey: TRAY_KEY,
...
export function useTrayRefreshMutation(q: TrayQuery) {
