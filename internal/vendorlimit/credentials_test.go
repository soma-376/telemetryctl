...
func TestLoadCodexCredential(t *testing.T) {
	t.Parallel()
	tests := []struct {
		name       string
		body       string
		...
		{
			name:     "정상 파일에서 토큰·계정을 읽는다",
			body:     codexAuthJSON(codexCanary, accountCanary),
			write:    true,
			wantAcct: accountCanary,
		},
		{
			name:  "account_id 가 없어도 토큰만 있으면 된다",
			body:  `{"tokens":{"access_token":"` + codexCanary + `"}}`,
			write: true,
		},
		{
			name:       "파일이 없으면 credential_missing",
		...
		{
			name:       "API 키 로그인은 구독 한도가 없는 계정이므로 credential_missing",
			body:       `{"OPENAI_API_KEY":"sk-proj-canary","tokens":null}`,
			write:      true,
			wantReason: ReasonCredentialMissing,
		},
		{
			name:       "tokens 가 없으면 credential_malformed",
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			cred, err := loadCodexCredential(home)
			if tc.wantReason != "" {
				if err == nil {
					t.Fatalf("실패해야 하는데 성공했다: %+v", cred)
				}
				...
				assertNoSecret(t, "오류 문자열", []string{err.Error()}, codexCanary, "sk-proj-canary", home)
				return
			}
			if err != nil {
				t.Fatalf("loadCodexCredential: %v", err)
			}
			if cred.token.reveal() != codexCanary {
				t.Errorf("토큰을 못 읽었다: %q", cred.token.reveal())
			}
			if cred.accountID != tc.wantAcct {
				t.Errorf("account_id = %q, want %q", cred.accountID, tc.wantAcct)
			}
		})
	}
}
