...
// TestEdgeCase_VendorUsageAPIFailureKeepsTrayUsable 은 벤더 사용량 API 가 죽었을 때
// 트레이가 어떻게 보이는지 고정한다.
//
// 조회는 **httptest 모의 서버** 로 간다. 실제 벤더 엔드포인트도, 실제 자격증명도 쓰지
// 않는다. 응답 → Result 매핑 자체는 internal/vendorlimit 이 소유하고 그 패키지의
// statusUpstream 테스트가 고정한다. 여기서 소유하는 것은 **화면 계약** 이다 —
// 한도가 비어도 로컬 숫자는 살아 있고, 스냅샷은 stale 이 아니며, 가장 빠듯한 한도는 없다.
func TestEdgeCase_VendorUsageAPIFailureKeepsTrayUsable(t *testing.T) {
	f := newFixture(t)
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			m.collect = func(ctx context.Context) vendorlimit.Snapshot {
				return probeFailingUsageAPI(ctx, srv.URL, tc.wantReason)
			}

			snap, err := m.Snapshot(context.Background(), TrayQuery{TZ: seoul})
			if err != nil {
				t.Fatalf("Tray: %v — 벤더 장애는 조회 실패가 아니다", err)
			}
			if hits == 0 {
...
// 옮긴다.
//
// **응답 해석 로직을 흉내 내지 않는다.** 여기서 하는 일은 "요청이 실제로 나갔고 실패했다"
// 를 만드는 것뿐이고, 상태 코드 → Reason 매핑의 정확성은 internal/vendorlimit 의
// transport_test.go 가 소유한다. 그래서 기대 Reason 을 인자로 받는다 — 이 파일이 그
// 매핑의 두 번째 정의가 되면 두 곳이 조용히 갈린다.
