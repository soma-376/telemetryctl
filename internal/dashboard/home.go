...
// Home 화면 (PROJ-88).
//
// today.go 의 Today 는 "지금이 속한 하루" 밖에 물을 수 없다. 여기서는 날짜를 인자로 받는
// 요약과 그 날짜의 최근 활동을 만든다. 비용 산정의 입력은 home_scan.go 가 읽는다.
//
// (패키지 머리말은 dashboard.go 에 있다. 이 주석은 파일 설명이라 package 절 뒤에 둔다.)
...
// 그보다 크거나 같다 (home_scan.go · internal/pricing). 카드에 나가는 것은 후자다.
