...
type Source interface {
	...
	// Refresh 는 벤더 한도를 다시 조회한 뒤 갱신된 스냅샷을 돌려준다.
	Refresh(ctx context.Context, q Query) (Snapshot, error)
}
...
// Refresh 는 벤더 한도를 다시 조회하게 한 뒤 갱신된 스냅샷을 받는다.
// 트레이의 "새로고침" 이 부른다.
//
// 갱신 명령이 실패해도 조회는 그대로 한다. 데몬이 꺼져 있다고 과거 스냅샷까지 못 보게 할
// 이유가 없다 — 그 사실은 Stale 과 monitoring.state 가 말한다.
func (c *Cache) Refresh(ctx context.Context, q Query) (Snapshot, error) {
	if _, err := dashboard.LoadLocation(q.TZ); err != nil {
		return Snapshot{}, err
	}
	c.mu.Lock()
	defer c.mu.Unlock()
	out, err := c.src.Refresh(ctx, q)
