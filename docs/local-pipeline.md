...
### 6.9.1 홈 화면의 기간 조회와 배선 (PROJ-168)

React 홈은 `internal/dashboard/home`의 `Builder.Snapshot(Query)`를 조회한다.
`Query{TZ, Start, End}`는 현지 날짜 양 끝을 포함하며 1~400일을 받는다.
시작·끝 날짜 또는 시간대가 잘못되면 `GET /v1/home?q=<Query JSON>`은 400을 반환한다.
수신기의 로컬 인증을 통과한 GET·HEAD만 허용한다. GUI는 기존 로컬 클라이언트와 Wails
`Dashboard.Home`을 통해 호출하며, SQLite를 직접 열지 않는다(ADR 0013).

| 선택 기간 | 차트 구간 |
|---|---|
| 1일 | 현지 자정부터 2시간 |
| 2일 | 각 날짜의 현지 자정부터 6시간 |
| 3~31일 | 현지 날짜별 |
| 32~120일 | 선택 시작일부터 7일씩 |
| 121~400일 | 달력 월별, 첫·마지막 월은 선택 범위로 자름 |

DST 전환일은 마지막 시간 구간을 다음 현지 자정에서 끊는다. v3 원본 저장소는 시간별
롤업을 강등하지 않으므로, 과거 하루를 선택해도 같은 시간 구간을 적용한다.
...
- 차트·벤더·요약의 토큰은 입력+출력이다. 캐시·추론 토큰을 다시 더하지 않는다.
- 비용은 보고값 우선, 누락 시 공시 가격표로 추정한다. 산정 불가 호출은 금액 합계에서
  제외하고 화면에 개수를 표시한다. 전부 산정 불가이면 `$0.00`로 표시하지 않는다.
- 활동 시간은 세션 시작일에 생애 전체 값을 귀속한다. 평균은 활동이 있는 차트 구간만
  분모에 넣는다. 미래·무활동 구간은 평균을 낮추지 않는다.
- 최근 활동은 선택 기간에 시작한 세션을 최신순으로 최대 7건 보여준다. 각 행의 토큰은
  세션 생애 전체다. 전체 세션 수와 진행 중 수는 목록의 7건 제한을 적용하지 않는다.
- 현재 활성 벤더는 선택 기간과 무관하다. 헤더의 토큰은 항상 오늘 조회에서 가져온다.
- UTC 정시 버킷에 기초한 토큰·활동의 구간 배분은 §6.9의 근사를 유지한다. 30·45분
  오프셋에서는 인접 구간으로 일부 값이 옮겨질 수 있으나 선택 기간 전체 합계는 유지된다.

프런트는 `['home', {tz, start, end}]`로 캐시를 나눈다. 같은 기간 조회의 갱신 실패에는
마지막 정상 응답과 오류 안내를 함께 표시하고, 다른 기간의 데이터로 대체하지 않는다.
네이티브 메인 창이 보이는 동안 30초마다 조회하고 숨기면 중단한다. 다시 열면 즉시
재조회한다. 같은 키를 쓰는 헤더와 홈은 React Query 캐시를 공유한다(ADR 0015·0029).
홈의 목 데이터는 제거하며, 수집하지 않은 요금제는 미확인으로 표시한다.
