...
### C. Unix domain socket 또는 Windows named pipe를 사용한다

- 장점: TCP 포트 대신 로컬 소켓·파이프를 쓰고 OS의 접근 권한으로 통신 대상을 제한할 수 있다.
- 단점: Unix domain socket과 Windows named pipe를 지원하려면 연결·권한 설정·종료 처리를
  플랫폼별로 구현하고 검증해야 한다. 기존 HTTP/JSON 계약을 유지하더라도 그 아래 전송 계층과
  주소 탐색을 별도로 연결해야 한다.
- 현재 HTTP를 유지하는 이유: 데몬은 이미 OTLP용 loopback HTTP 수신기를 소유한다.
  `receiver.Options.LocalAPI`에 조회 핸들러를 연결하면 기존 리스너·인증·종료 경로를 재사용할 수
  있고, GUI도 Go의 HTTP 클라이언트로 요청·취소·타임아웃을 처리한다. 현재 트레이 조회를 위해
  별도 IPC 리스너와 OS별 구현을 추가할 필요가 없다.
- 재검토 조건: 로컬 사용자별 접근 제어가 필요하거나, TCP 포트 사용이 배포 환경의 제약이 되면
  소켓·파이프 전송과 권한 모델을 함께 검토한다. HTTP가 성능이나 보안에서 항상 우월하다는
  판단은 아니며, 전송 방식 간 성능 비교 측정은 하지 않았다.

### D. 공유 메모리를 사용한다

- 장점: 두 프로세스가 같은 메모리 영역에서 스냅샷을 전달하도록 구성할 수 있다.
- 단점: 스냅샷의 쓰기 완료 여부, 동시 접근, 형식 버전과 프로세스 종료 시 정리를 별도로
  설계해야 한다. 수동 갱신 명령과 완료·실패 응답을 전달하는 동기화 방식도 필요하다.
- 현재 채택하지 않는 이유: 이 API는 트레이 스냅샷 조회와 수동 갱신의 요청·응답이다.
  직렬화·전송 비용이 병목이라는 측정 근거가 없는 상태에서 공유 메모리 프로토콜을 추가하는
  비용보다 기존 HTTP 요청·응답 경로를 재사용하는 편이 단순하다.
