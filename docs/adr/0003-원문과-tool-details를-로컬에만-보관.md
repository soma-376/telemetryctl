...
> 로컬 `events.payload`의 무정제 수신 내용 보관은 [ADR 0025](0025-이벤트별-수신-payload를-로컬에-보관한다.md)가 속성 allowlist와 토큰 저장 금지를 부분 대체한다. 정규화 컬럼·상위 전송 규칙은 유지한다.
...
> [ADR 0010](0010-식별-정보를-로컬에만-저장한다.md)**으로 대체되었다.**
> 현재 스키마가 이 값들을 요구하는 컬럼을 갖기 때문이다.
