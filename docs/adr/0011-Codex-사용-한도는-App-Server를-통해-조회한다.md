...
## Status
Accepted — 부분 대체: [ADR 0014](0014-한도-갱신-억제를-등급별-쿨다운으로-나눈다.md)가 단일 쿨다운 결정을 데몬 주기 갱신과 수동 10초 제한으로 대체한다. App Server 채택·패키지 경계·자격증명 취급·오류 격리 결정은 유효하다.
...
- 수동 갱신의 첫 요청은 두 벤더를 즉시 조회한다. 동시에 들어온 자동·수동 요청은 진행 중인
  한 번의 결과를 공유한다. **대체됨(ADR 0014)** — 성공 완료 뒤 10초 동안의 요청이 추가 외부 호출
  없이 최신 스냅샷을 반환한다는 부분은 수동 요청의 10초 제한으로 바뀌었다.
  자동 갱신은 데몬 기동·5분 틱이 담당하고 창 열기는 스냅샷 조회만 한다.
