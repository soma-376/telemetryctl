...
## Status
Accepted — 부분 대체: [ADR 0014](0014-한도-갱신-억제를-등급별-쿨다운으로-나눈다.md)가 단일 10초 쿨다운 결정을 등급별 쿨다운으로 대체한다. App Server 채택·패키지 경계·자격증명 취급·오류 격리 결정은 유효하다.
...
- 수동 갱신의 첫 요청은 두 벤더를 즉시 조회한다. 동시에 들어온 자동·수동 요청은 진행 중인
  ...
  없이 최신 스냅샷을 반환한다는 부분은 등급별 쿨다운으로 대체됐다. 10초는 수동 새로고침 전용이 되고,
  자동 트리거와 창 열기에는 더 긴 쿨다운이 걸린다.
